...
<!-- 				<view class="icon-btn">
		...
		<view class="footer">
			<view class="icons">
				...
				<view class="box" @tap="toMakePhone">
			...
			<view class="btn">
				...
				<navigator :url="'../signup/signup?id='+goodsData.id"  class="buy">立即报名</navigator>
			</view>
		</view>
		<!-- share弹窗 -->
...
export default {
	...
	onLoad(option) {
		...
		// console.log(option.cid); //打印出上个页面传递的参数李杰。
		//  改标题
		var self=this;
		...
		//去请求商品内容 李杰
		uni.request({
			url:self.$url+'v1.goodslist/check_one',
			data:{id:option.cid},
			method:'POST',
			success:function(res){
				self.goodsData=res.data;
				// console.log(self.goodsData);
				self.descriptionStr=res.data.content;
			}
		})
